    ...
    public class ProcesoAbuelo {
        ...
        public static void main(String[] args) {
            ...
            for (int i = 0; i < 2; i++) {
                // Construir el comando con 8 equipos como argumentos
                List<String> command = new ArrayList<>();
                command.add("java");
                command.add("ProcesoPadre");
                for (int j = 0; j < 8; j++) {
                    String equipo = equipos.get(i * 8 + j);
                    command.add(equipo);
                    equipos.set(i * 8 + j, "-1"); // Marcar el equipo seleccionado con -1
                }
            ...
            for (ProcessBuilder pb : procesos) {
                try {
                    ...
                    // Capturar la salida del proceso intermedio
                    InputStream is = process.getInputStream();
                    BufferedReader reader = new BufferedReader(new InputStreamReader(is));
                    String line;
                    while ((line = reader.readLine()) != null) {
                        System.out.println("Salida del proceso intermedio: " + line);
                    }
